...
export default {
    ...
    execute: async (
        ...
        const guildPolls: GuildPolls = guildPollsDoc.exists
          ?  !(guildPollsDoc.data() as GuildPolls).polls ? { polls: [] }
        ...
        const data: Poll = {
            ...
            uuid: (( (guildPolls.polls?.length ?? 0) !== 0 ? parseInt(guildPolls.polls.at(-1)!.uuid) : 0) + 1).toString().padStart(3, "0"),
            active: true,
            quorum: quorum,
        ...
        try {
            const message = await interaction.channel!.send({ embeds: [ pollEmbed(data, interaction.user) ], components: [ castPollVoteButtons() ] });
            ...
            if (message.channel instanceof TextChannel) {
                try {
                    await message.channel.threads.create({
                        ...
                        autoArchiveDuration: message.guild!.premiumTier === "NONE" ? 1440 : 4320
                    })
                } catch (error) {
                    await interaction.followUp({ embeds: [ primaryEmbed("Could not create a thread", "Please make sure I have permission to create threads.") ] })
                }
            }
            setTimeout(() => { archivePoll(lunarAssistant, message, data) }, data.endsAt - Date.now());
        } catch (error) {
            await interaction.followUp({ embeds: [ primaryEmbed("Could not send poll message", "Please make sure I have permission to send messages on that channel.") ] })
            return;
        }
        ...
        await interaction.followUp({ embeds: [ primaryEmbed(undefined, "Proposal Successfully Created!") ], ephemeral: true })
    }
}